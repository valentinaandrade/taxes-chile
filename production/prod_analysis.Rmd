...
set_theme(base = theme_hc(),
          ...
          panel.gridcol.y = "white")
ggplot(frqdata, aes(x=2, y=raw.prc, fill=tax,label=raw.prc)) +
  geom_bar(stat = "identity") + 
  ...
  geom_text(data = filter(frqdata,tax %in% levels(frqdata$tax)[1:3]),
             size = 4.5,
             mapping =aes(x=2, y=raw.prc,label=paste0(round(raw.prc,0),"%")),
             position = position_stack(vjust = 0.5),
             colour = "white")+
  # scale_y_continuous(name = NULL) +
  # scale_x_discrete(name = "Preferencias por impuestos a los más ricos")+
  labs(caption = fuente,
       y="Preferencias por impuestos a los más ricos",
       x=NULL)+
  theme(legend.position = "top") +
  scale_fill_grey(name = NULL) +
  guides(fill = guide_legend(reverse=T))+
  theme(axis.title.y = element_text(angle = 90),
        ...
        legend.justification = "center",
        strip.text.x = element_text(size = 18),
        strip.background = element_rect(color = "black",linetype = "solid")) ->dnt_pref;dnt_pref

ggsave(dnt_pref,filename = "output/images/dnt_pref.png",device = "png",width = 25,height = 15,dpi = 300,units = "cm")
...
taxperc_lik<- ggplot(frqdata, aes(x=hwork, y=raw.prc, fill=taxperc,label=raw.prc)) +
  ...
  theme(axis.title.y = element_text(angle = 90),
        ...
        legend.justification = "center",
        strip.text.x = element_text(size = 18),
        strip.background = element_rect(color = "black",linetype = "solid"));taxperc_lik
...
set_theme(base = theme_hc(),
          ...
          panel.minor.gridcol = "white",
          panel.gridcol.y = "white")
ggplot(frqdata, aes(x=2, y=raw.prc, fill=taxperc,label=raw.prc)) +
  geom_bar(stat = "identity") + 
  coord_polar("y", start = 400) +
  facet_grid(~year) +
  xlim(.2,3)+
  ...
  # scale_x_discrete(name = "Preferencias por impuestos a los más ricos")+
  labs(caption = fuente,
       y="Percepción de pago de impuestos a los más ricos",
       x=NULL)+
  theme(legend.position = "top") +
  scale_fill_grey(name = NULL) +
  guides(fill = guide_legend(reverse=T))+
  theme(axis.title.y = element_text(angle = 90),
        ...
        legend.justification = "center",
        strip.text.x = element_text(size = 18),
        strip.background = element_rect(color = "black",linetype = "solid")) ->dnt_perc;dnt_perc



ggsave(dnt_perc,filename = "output/images/dnt_perc.png",device = "png",width = 25,height = 15,dpi = 300,units = "cm")
...
data <- data %>% mutate(hwork=car::recode(hwork,recodes = "c('No es importante','No muy importante')='No es importante/No muy importante';'Bastante importante'='Bastante importante';c('Muy importante','Esencial')='Muy importante/Esencial'",as.factor = T))
...
frqdata$hwork <- fct_relevel(frqdata$hwork,levels(frqdata$hwork)[c(3,1,2)])
...
rownames <- c("A. Pref. Impuestos","B. Pref. Redistributivas","C. Igual. Salud","D. Igual. Educación")
colnames <- c("(A)","(B)","(C)","(D)")
rownames(cor99a) <- rownames
colnames(cor99a) <- colnames
rownames(cor09a) <- rownames
colnames(cor09a) <- colnames
rownames(cor19a) <- rownames
colnames(cor19a) <- colnames
...
corrplot(
  ...
  tl.cex =0.75,
  number.cex = 1.5)
text(-0.5, 0.95, "A. Preferencia por mayores impuestos a los más ricos",font=6,cex=0.75,adj=0)
text(-0.5, 0.80, "B. Preferencia redistributivas",font=6,adj=0,cex=0.75)
text(-0.5, 0.65, "C. Justicia en el acceso a salud por los más ricos",font=6,adj=0,cex=0.75)
text(-0.5, 0.5, "D. Justicia en el acceso a educación por los más ricos",font=6,adj=0,cex=0.75)
dev.off()
...
cl16[cl16==9] <- NA
cl16 <- cl16 %>%
  mutate_at(vars(gsalud,geduc,gpension, gunemp), funs(as.numeric(.))) %>% 
  mutate_at(vars(gsalud,geduc,gpension, gunemp),
                    funs(car::recode(.,  recode="1:2='Gastar más';4:5='Gastar menos';3='Gastar lo mismo'", as.factor = T, 
                             levels = c("Gastar más", "Gastar menos","Gastar lo mismo")))) %>% na.omit()
...
gov1<- plot_likert(cl16[,c("gsalud","geduc","gpension","gunemp")],cat.neutral = 3, catcount = 2, values="sum.outside", 
  show.prc.sign = TRUE, legend.pos = "bottom",
                   axis.labels = c("Salud","Educación","Mejores pensiones","Beneficio desempleo"),show.n = F,reverse.scale = FALSE,weight.by = cl16$WEIGHT)  + 
  labs(title ="Áreas de gasto del gobierno. Dígame si a Ud. le gustaría que se gastara más o menos en cada una de ellas.",
       subtitle = "'Mucho más', podría ser necesario aumentar los impuestos.", 
       caption = "International Social Survey Programme - Role of Goverment 2016 (n=1349). Análisis incluyen ponderadores.")  + 
  theme(axis.text=element_text(size=12, face="bold"),
        axis.title=element_text(size=14,face="bold"),
        plot.title=element_text(hjust=0.5,size = 14,face = "bold"),
        plot.subtitle = element_text(hjust=0.5,size = 12,face = "italic")) + scale_fill_grey(labels = c("Gastar lo mismo","Gastar más","Gastar menos" ),breaks=c("neutral",2,1))
gov1
