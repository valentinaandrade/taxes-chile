...
**Libraries**
```{r}
pacman::p_load(tidyverse, sjlabelled,sjPlot,sjmisc,knitr,kableExtra,questionr,RColorBrewer,cowplot,ggthemes,ggmosaic,ggrepel, car) 
```
...
```{r}
fuente <- "Fuente: ISSP (1999, 2009, 2019). Análisis incluyen ponderadores."
fuente2 <- "Fuente: ISSP (2009, 2019). Análisis incluyen ponderadores."
breaks <- c(1:5)
```
...
pref_income <- issp %>%
  ...
  theme(legend.position = "bottom",
        panel.grid.major.y =element_line(linetype = 3),axis.text.y.left = element_text(hjust = 1))
...
pref_ess<- issp %>%
  ...
  scale_y_continuous(name = NULL,
                    ...
                    breaks = c(1:5),
                    sec.axis = sec_axis(~.)) +
  scale_x_discrete(name = "Estatus Subjetivo",labels=c("Bajo",2:8,"Alto")) +
  guides(shape = guide_legend(title = NULL),
         color = guide_legend(title = NULL)) +
  theme(legend.position = "right",axis.text.y.left = element_text(hjust = 1)) +
  labs(caption = fuente) +
  scale_color_grey() 
pref_ess
...
set_theme(base = theme_hc(),
          ...
          geom.label.size = 4, #size of sum outside
          legend.just = 0.35, 
          legend.size = 1.0, axis.tickscol = "gray")
...
data1 %>% 
  ...
  facet_grid(~year) +
  scale_y_continuous(breaks = c(0.05,0.5,0.97),labels = str_wrap(levels(data1$tax)[c(2,1,3)],width = 15))+
  # labs(caption = fuente2)+
...
data1 %>% 
  ...
  geom_label(data = layer_data(last_plot(), 1),
             aes(x = (xmin + xmax)/ 2,
                 ...
                 label = paste0(round((.wt/sum(.wt))*100,1),"%"))) +
  facet_grid(~year) +
  # scale_y_continuous(breaks = c(0,0.5,1),labels = str_wrap(levels(data1$tax),width = 15))+
...
data1 %>% 
  ...
  geom_label(data = layer_data(last_plot(), 1),
             aes(x = (xmin + xmax)/ 2,
                 ...
                 label = paste0(round((.wt/sum(.wt))*100,1),"%"))) +
  facet_grid(~year) +
  scale_y_continuous(breaks = c(0.1,0.5,0.9),labels = str_wrap(levels(data1$tax),width = 15))+
  # labs(caption = fuente2)+
  guides(fill = guide_legend(title = NULL)) +
  theme(axis.title.y = element_text(angle = 90), 
        axis.title.y.left = element_text(size = 18),
...
red_decil <- issp %>%
  ...
  scale_y_continuous(
    ...
    breaks = c(1:5),
    sec.axis = sec_axis(~.,breaks = 1:5)) +
  scale_x_discrete(name = NULL,labels=str_wrap(levels(issp$decil),width = 10)) +
  guides(shape = guide_legend(title = NULL),
         color = guide_legend(title = NULL)) +
  theme(legend.position = "right",
        legend.key = element_rect(fill = "White"),axis.text.y.left = element_text(hjust = 1)) +
